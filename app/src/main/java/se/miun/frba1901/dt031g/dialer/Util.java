...
public class Util {
    ...
    /** Array containing all symbols that are voiced */
    ...
    public static boolean defaultVoiceExist(Context context) {
        File[] soundFiles = getDirForDefaultVoice(context).listFiles();
        if(soundFiles == null || soundFiles.length == 0)
            return false; // Return false if soundFiles is null or is empty
